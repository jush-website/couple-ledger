...
const GoldChart = ({ data, period, loading }) => {
    ...
    const chartData = useMemo(() => {
        ...
        if (period === '3m') return data.slice(-90); // Use available history
        // Default (Instant) - could show last few data points
...
export default function CoupleLedgerApp() {
  ...
  const [goldPeriod, setGoldPeriod] = useState('5d'); // 1d, 5d, 3m
...
const GoldView = ({ transactions, goldPrice, history, period, setPeriod, onAdd, onEdit, onDelete, loading, error, onRefresh, role }) => {
                          ...
                          <div key={date}>
                             <div className="bg-gray-50/50 px-4 py-2 flex justify-between items-center border-b border-gray-50">
                                ...
                                <div className="flex gap-3 text-xs font-bold">
                                    <span className="text-blue-600">👦 {formatMoney(daily.bf)}</span>
                                    <span className="text-pink-600">👧 {formatMoney(daily.gf)}</span>
                                </div>
                             </div>
                             {items.map(t => (
        ...
        return (
          <div key={jar.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center relative z-10">
                <div className="flex gap-2">
                    <div className="flex items-center gap-1 bg-blue-50 text-blue-600 px-2 py-1 rounded-lg text-xs font-bold" title="男友貢獻">
                        <span>👦</span>
                        <span>{formatMoney(jar.contributions?.bf || 0)}</span>
                    </div>
                    <div className="flex items-center gap-1 bg-pink-50 text-pink-600 px-2 py-1 rounded-lg text-xs font-bold" title="女友貢獻">
                        <span>👧</span>
                        <span>{formatMoney(jar.contributions?.gf || 0)}</span>
                    </div>
                </div>
                <div className="flex gap-2">
                ...
                {initialData && (
                    <div className="bg-orange-50 p-3 rounded-xl border border-orange-100">
                        ...
                        <p className="text-xs text-orange-600 mt-2">
                            {isArchived ? '此帳本將移至歷史區，主畫面將隱藏。' : '此帳本目前正在使用中。'}
                        </p>
                    </div>
                )}
            ...
            {step === 'upload' && !errorMsg && (
                <div className="flex flex-col items-center justify-center h-64 gap-4">
                    <label className="w-full h-full flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-2xl bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors">
                        ...
                        <span className="text-xs text-gray-400 mt-2">支援自動翻譯與分類</span>
            ...
            {step === 'analyzing' && !errorMsg && (
                <div className="flex flex-col items-center justify-center h-64 gap-4">
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-1">AI 正在辨識品項並翻譯成中文</p>
                    </div>
                </div>
            )}
            ...
            {step === 'review' && scannedData && !errorMsg && (
                <div className="space-y-4">
                    ...
                    <div className="space-y-2 max-h-[50vh] overflow-y-auto">
                        <div className="text-xs text-gray-400 px-1">請勾選要匯入的項目：</div>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-[10px] text-gray-500 flex gap-1 items-center">
                                            <span className="bg-gray-200 px-1 rounded">{CATEGORIES.find(c=>c.id===item.category)?.name || '其他'}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="border-t border-gray-100 pt-3">
                        <div className="flex justify-between items-center mb-3">
                            <span className="text-sm font-bold text-gray-500">選取金額合計</span>
