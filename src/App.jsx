...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Auth Initialization with explicit error handling
    ...
    const unsubscribe = onAuthStateChanged(auth, (u) => {
        setUser(u);
        if (u) setAuthError(null); // Clear error if successfully authenticated
    });
  ...
  const fetchGoldPrice = async () => {
      ...
      try {
          ...
          try {
              // --- Fallback Mechanism using AllOrigins if Vercel backend fails ---
  ...
  useEffect(() => {
    if (!user) return;
    setDbError(null); // Reset dbError
    
    // Centralized Firestore Snapshot Error Handler
    const handleSnapError = (err) => {
        console.error("Firestore DB Error:", err);
        if (err.code === 'permission-denied') {
            setDbError("資料庫權限被拒絕：請確認是否已啟用「匿名登入」且 Firestore 安全性規則正確。");
        }
    };
