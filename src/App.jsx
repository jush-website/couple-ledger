...
export default function CoupleLedgerApp() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-gray-800 relative">
      {/* Toast Notification */}
      {toast && (
        <div className="fixed top-6 left-1/2 transform -translate-x-1/2 bg-gray-900/90 backdrop-blur text-white px-6 py-3 rounded-full shadow-xl z-[100] flex items-center gap-3 animate-in fade-in slide-in-from-top-5 duration-300">
...
const RoleSelection = ({ onSelect }) => (
  <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-pink-100 to-blue-100 p-6">
    <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm text-center space-y-8">
      <div>
  ...
  <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-black/40 backdrop-blur-[2px] animate-in fade-in duration-200">
    <div className="bg-white w-full max-w-xs rounded-3xl p-6 shadow-2xl scale-100 animate-in zoom-in-95 duration-200">
      ...
      <div className="flex gap-3">
        ...
        <button 
          onClick={onConfirm}
          className={`flex-1 py-3 rounded-xl font-bold text-sm text-white shadow-lg transition-transform active:scale-95 ${isDanger ? 'bg-red-500 hover:bg-red-600' : 'bg-blue-500 hover:bg-blue-600'}`}
        >
          確定
        </button>
      </div>
  ...
  const debtSummary = useMemo(() => {
    ...
    transactions.forEach(t => {
      ...
      if (t.category === 'repayment') {
        if (t.paidBy === 'bf') bfLent -= amount; 
        else bfLent += amount; 
      } else {
        let gfShare = 0;
        let bfShare = 0;
        if (t.splitDetails) {
          gfShare = Number(t.splitDetails.gf);
          bfShare = Number(t.splitDetails.bf);
        } else {
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 text-center relative overflow-hidden">
        ...
        <div className="flex items-center justify-center gap-3">
          {Math.abs(debtSummary) < 1 ? (
            ...
            <>
              <span className={`text-xl font-bold ${debtSummary > 0 ? 'text-pink-500' : 'text-blue-500'}`}>
                {debtSummary > 0 ? '女朋友' : '男朋友'}
              </span>
              <span className="text-gray-400 text-sm">欠</span>
              <span className={`text-xl font-bold ${debtSummary > 0 ? 'text-blue-500' : 'text-pink-500'}`}>
                {debtSummary > 0 ? '男朋友' : '女朋友'}
              </span>
              <div className="text-3xl font-extrabold text-gray-800 ml-2">
                {formatMoney(Math.abs(debtSummary))}
              </div>
            </>
          )}
        </div>
        {Math.abs(debtSummary) > 0 && (
           <p className="text-xs text-gray-400 mt-2">
             {debtSummary > 0 && role === 'gf' ? "記得還錢喔！" : ""}
             {debtSummary < 0 && role === 'bf' ? "記得還錢喔！" : ""}
           </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center gap-2 text-gray-500 mb-1">
            <Calendar size={16} />
            <span className="text-xs">本月總支出</span>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center items-center cursor-pointer hover:bg-gray-50 transition-colors" onClick={onAdd}>
          <div className={`p-3 rounded-full mb-2 ${role === 'bf' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
            <Plus size={24} />
          </div>
              ...
              <div key={t.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center group">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-400 flex gap-2">
                      <span>{t.date}</span>
                      <span className={`${t.paidBy === 'bf' ? 'text-blue-500' : 'text-pink-500'}`}>
                        {t.paidBy === 'bf' ? '男友付' : '女友付'}
                      </span>
                      <span>•</span>
                      <span>
                        {t.category === 'repayment' ? '還款' : 
                         (t.splitDetails ? 
                           (t.splitDetails.bf > 0 && t.splitDetails.gf > 0 ? '分帳' : 
                            t.splitDetails.bf > 0 ? '男友個人' : '女友個人') 
                           : '一般支出')}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-3">
                   <div className="text-right">
                     <span className={`font-bold block ${t.category === 'repayment' ? 'text-green-500' : 'text-gray-800'}`}>
                       {t.category === 'repayment' ? '+' : '-'}{formatMoney(t.amount)}
                     </span>
                     {t.splitDetails && t.category !== 'repayment' && (
                       <span className="text-[10px] text-gray-400 block">
                         (B:{formatMoney(t.splitDetails.bf)} G:{formatMoney(t.splitDetails.gf)})
                       </span>
                     )}
                   </div>
                   
                   <div className="flex gap-1">
                     <button onClick={() => onEdit(t)} className="p-1.5 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-full transition-colors">
                       <Pencil size={16} />
                     </button>
                     <button onClick={() => onDelete(t.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-full transition-colors">
                       <Trash2 size={16} />
                     </button>
                   </div>
                </div>
...
const Statistics = ({ transactions }) => {
  ...
  const statsData = useMemo(() => {
    const filtered = transactions.filter(t => {
      if (t.category === 'repayment') return false;
      const d = new Date(t.date);
        ...
        {statsData.map((item, idx) => (
          <div key={idx} className="bg-white p-3 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <div className="flex items-center gap-4">
              <span className="text-gray-800 font-bold">{formatMoney(item.value)}</span>
              <span className="text-gray-400 text-xs w-10 text-right">{((item.value / total) * 100).toFixed(0)}%</span>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2">
        ...
        <button onClick={onAdd} className="flex items-center gap-1 bg-gradient-to-r from-teal-400 to-emerald-400 text-white px-4 py-2 rounded-full shadow-md text-sm font-bold active:scale-95 transition-transform">
          <Plus size={16} /> 新增罐子
        </button>
      </div>
      ...
      {jars.length === 0 && (
         <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-gray-200">
           ...
           <p className="text-gray-400">還沒有存錢罐，一起設立一個夢想吧！</p>
         </div>
      )}
          ...
          <div key={jar.id} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 relative group overflow-hidden">
             ...
             <div className="absolute top-4 right-4 flex gap-2 z-10">
               <button 
                  ...
                  className="p-1.5 bg-white/80 hover:bg-blue-100 text-gray-400 hover:text-blue-500 rounded-full transition-colors shadow-sm"
               >
                 <Pencil size={16} />
               </button>
               <button 
                  onClick={(e) => { e.stopPropagation(); onDelete(jar.id); }}
                  className="p-1.5 bg-white/80 hover:bg-red-100 text-gray-400 hover:text-red-500 rounded-full transition-colors shadow-sm"
               >
                 <Trash2 size={16} />
               </button>
             </div>
             ...
             <div className="flex items-center gap-4 mb-4">
               ...
               <div>
                 ...
                 <div className="text-xs text-gray-400 flex gap-2">
                   <span>目標: {formatMoney(jar.targetAmount)}</span>
                 </div>
               </div>
             </div>
             ...
             <div className="relative h-4 bg-gray-100 rounded-full overflow-hidden mb-3">
               <div 
                 className="absolute top-0 left-0 h-full bg-gradient-to-r from-yellow-300 to-orange-400 transition-all duration-1000 ease-out"
             ...
             <div className="flex justify-between items-end">
                <div>
                   ...
                   <div className="text-xs text-gray-400 mt-1 flex gap-2">
                      <span className="text-blue-500">👦 {formatMoney(jar.contributions?.bf || 0)}</span>
                      <span className="text-pink-500">👧 {formatMoney(jar.contributions?.gf || 0)}</span>
                   </div>
                </div>
                <button 
                  onClick={() => onDeposit(jar.id)}
                  className="bg-gray-800 text-white px-4 py-2 rounded-xl text-sm font-bold shadow-lg active:scale-95 transition-transform"
                >
                  存入
                </button>
             </div>
  ...
  <div className="space-y-4">
    <div className="bg-white p-6 rounded-3xl shadow-sm">
       ...
       <div className="flex items-center gap-4 mb-6">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white text-xl ${role === 'bf' ? 'bg-blue-500' : 'bg-pink-500'}`}>
            {role === 'bf' ? 'BF' : 'GF'}
          </div>
          <div>
            <div className="font-bold">當前身分：{role === 'bf' ? '男朋友' : '女朋友'}</div>
            <div className="text-xs text-gray-400">ID: {localStorage.getItem('couple_app_role')}</div>
          </div>
       </div>

       <button 
         onClick={onLogout}
         className="w-full py-3 bg-red-50 text-red-500 rounded-xl font-medium flex items-center justify-center gap-2"
       >
         <LogOut size={18} /> 重選身分 / 登出
       </button>
    </div>
    
    <div className="bg-white p-6 rounded-3xl shadow-sm text-center">
       <p className="text-gray-400 text-sm">我們的小金庫 v1.5</p>
       <p className="text-gray-300 text-xs mt-2">Designed for Couples</p>
    </div>
  </div>
);
...
const AddTransactionModal = ({ onClose, onSave, currentUserRole, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (!isRepay) {
         if (initialData.splitType === 'custom_amount') {
           setSplitMode('amount');
           setBfAmount(initialData.splitDetails?.bf || 0);
           setGfAmount(initialData.splitDetails?.gf || 0);
         } else if (initialData.splitType === 'custom_percent') {
           setSplitMode('percent');
           const total = Number(initialData.amount);
           const bf = initialData.splitDetails?.bf || 0;
           setBfPercent(Math.round((bf / total) * 100));
         } else if (initialData.splitType === 'bf_personal') {
           setSplitMode('amount');
           setBfAmount(initialData.amount);
           setGfAmount(0);
         } else if (initialData.splitType === 'gf_personal') {
           setSplitMode('amount');
           setBfAmount(0);
           setGfAmount(initialData.amount);
  ...
  useEffect(() => {
    ...
    if (splitMode === 'amount') {
      if (bfAmount !== '' && gfAmount !== '') {
         const currentTotal = Number(bfAmount) + Number(gfAmount);
         if (currentTotal !== total) { setBfAmount(total/2); setGfAmount(total/2); }
  ...
  const handleSubmit = () => {
    ...
    if (isRepayment) {
      ...
      if (splitMode === 'equal') {
        splitDetails = { bf: total / 2, gf: total / 2 };
        splitType = 'shared';
      } else if (splitMode === 'percent') {
        const bf = Math.round(total * (bfPercent / 100));
        splitDetails = { bf: bf, gf: total - bf };
        splitType = 'custom_percent';
      } else if (splitMode === 'amount') {
        const bf = Number(bfAmount);
        const gf = Number(gfAmount);
        if (Math.abs(bf + gf - total) > 1) {
          alert(`分帳金額總和 (${bf+gf}) 必須等於總支出金額 (${total})`);
          return;
        }
        splitDetails = { bf, gf };
    ...
    onSave({
      amount: total,
      note: isRepayment ? '還款' : note,
      category: isRepayment ? 'repayment' : category,
      paidBy: payer,
      splitType,
      splitDetails,
      date
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in slide-in-from-bottom-10 h-[85vh] sm:h-auto overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold">{initialData ? '編輯紀錄' : (isRepayment ? '還款給對方' : '記一筆')}</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
               ...
               <input 
                 ...
                 className="w-full bg-gray-50 p-3 pl-10 rounded-xl text-2xl font-bold outline-none focus:ring-2 focus:ring-blue-100"
                 placeholder="0"
          ...
          {!initialData && (
            <div className="flex gap-2 p-1 bg-gray-100 rounded-xl">
               <button onClick={() => setIsRepayment(false)} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${!isRepayment ? 'bg-white shadow-sm text-gray-800' : 'text-gray-400'}`}>一般支出</button>
               <button onClick={() => { setIsRepayment(true); setCategory('repayment'); }} className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${isRepayment ? 'bg-white shadow-sm text-green-600' : 'text-gray-400'}`}>債務還款</button>
            </div>
          )}
          ...
          {!isRepayment && (
            <>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                  {CATEGORIES.filter(c => c.id !== 'repayment').map(c => (
                    <button
                      ...
                      className={`flex flex-col items-center gap-1 min-w-[60px] p-2 rounded-xl transition-all ${category === c.id ? 'bg-gray-800 text-white scale-105' : 'bg-gray-50 text-gray-500'}`}
                    >
                      <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: category === c.id ? 'rgba(255,255,255,0.2)' : c.color }}>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-xs text-gray-400 font-bold ml-1">誰付錢？</label>
                    <div className="flex gap-2 mt-1">
                       <button onClick={() => setPayer('bf')} className={`flex-1 py-2 rounded-lg text-sm font-bold border ${payer === 'bf' ? 'bg-blue-50 border-blue-500 text-blue-600' : 'border-gray-200 text-gray-400'}`}>男友</button>
                       <button onClick={() => setPayer('gf')} className={`flex-1 py-2 rounded-lg text-sm font-bold border ${payer === 'gf' ? 'bg-pink-50 border-pink-500 text-pink-600' : 'border-gray-200 text-gray-400'}`}>女友</button>
                    </div>
                 </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
                ...
                <div className="flex gap-1 bg-gray-200 p-1 rounded-lg mb-4">
                  {['equal', 'percent', 'amount'].map(m => (
                    <button key={m} onClick={() => setSplitMode(m)} className={`flex-1 py-1.5 rounded-md text-xs font-bold ${splitMode === m ? 'bg-white shadow text-gray-800' : 'text-gray-500'}`}>
                      {m === 'equal' ? '均分' : m === 'percent' ? '比例' : '金額'}
                    </button>
                  ))}
                </div>
                ...
                {splitMode === 'equal' && (
                  <div className="flex gap-2">
                     <button onClick={() => { setSplitMode('amount'); setBfAmount(Number(amount)); setGfAmount(0); }} className="flex-1 py-2 border border-blue-200 text-blue-500 rounded-lg text-xs font-bold hover:bg-blue-50">男友全出</button>
                     <button onClick={() => { setSplitMode('amount'); setBfAmount(0); setGfAmount(Number(amount)); }} className="flex-1 py-2 border border-pink-200 text-pink-500 rounded-lg text-xs font-bold hover:bg-pink-50">女友全出</button>
                  </div>
                )}
                {/* Simplified Percent/Amount inputs for brevity, same logic as before */}
                {splitMode === 'amount' && (
                  <div className="flex gap-3">
                    <input type="number" value={bfAmount} onChange={(e) => { setBfAmount(e.target.value); setGfAmount(Number(amount)-e.target.value); }} className="flex-1 p-2 rounded-lg text-center border font-bold" placeholder="男友" />
                    <input type="number" value={gfAmount} onChange={(e) => { setGfAmount(e.target.value); setBfAmount(Number(amount)-e.target.value); }} className="flex-1 p-2 rounded-lg text-center border font-bold" placeholder="女友" />
                  </div>
                )}
              </div>
            </>
          ...
          <button onClick={handleSubmit} className="w-full bg-gray-900 text-white py-4 rounded-2xl font-bold text-lg shadow-lg active:scale-95 transition-transform mt-4">
            {initialData ? '儲存修改' : (isRepayment ? '確認還款' : '儲存紀錄')}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="relative mb-4">
           <DollarSign size={20} className="absolute left-3 top-3 text-gray-400" />
           <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-gray-50 p-3 pl-10 rounded-xl text-2xl font-bold outline-none focus:ring-2 focus:ring-blue-100" placeholder="0" autoFocus />
        </div>
        <div className="flex gap-2">
            <button onClick={onClose} className="flex-1 py-3 text-gray-500 font-bold">取消</button>
            <button onClick={() => { if(amount) onConfirm(jar.id, amount, role); }} className="flex-1 py-3 bg-gray-800 text-white rounded-xl font-bold shadow-lg">確認存入</button>
        </div>
      </div>
    </div>
  );
